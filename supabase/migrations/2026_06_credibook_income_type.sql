-- ============================================================================
-- Skupy POS — CREDIBOOK: JENIS PEMASUKKAN (income_type)
-- ----------------------------------------------------------------------------
-- Tambah kolom income_type pada credibook_income. Nilai:
--   omzet   → pendapatan usaha → IKUT Omset + Uang Masuk + Saldo + Arus Kas +
--             Total Aset + Kekayaan Bersih.
--   refund  → uang kembali → Uang Masuk + Saldo + Arus Kas + Total Aset +
--             Kekayaan Bersih, TIDAK ikut Omset / Profit Bruto.
--   capital → modal tambahan → Uang Masuk + Saldo + Total Aset (+ Kekayaan,
--             karena ekuitas naik). TIDAK ikut Omset / Laba / Profit Bruto.
--   other   → pemasukkan lainnya → Uang Masuk + Saldo + Arus Kas. TIDAK Omset.
--
-- INTI: hanya income_type='omzet' yang menambah OMSET. Semua jenis tetap
-- menambah kas (Saldo/Uang Masuk/Total Aset/Kekayaan) karena uang benar masuk.
-- Idempotent. Jalankan SETELAH 2026_06_credibook.sql.
-- ============================================================================

ALTER TABLE public.credibook_income
  ADD COLUMN IF NOT EXISTS income_type text NOT NULL DEFAULT 'other';

-- Data lama (dibuat saat Credibook masih full-omset) → tetap dihitung Omset.
UPDATE public.credibook_income
   SET income_type = 'omzet'
 WHERE income_type IS NULL OR income_type = 'other';

CREATE INDEX IF NOT EXISTS idx_credibook_income_type ON public.credibook_income(income_type);

-- ───── acc_dashboard: hanya jenis 'omzet' yang menambah penjualan/Omset ─────
...
RETURNS json LANGUAGE sql STABLE AS $$
  ...
  -- CREDIBOOK: pemasukkan manual (semua jenis = kas masuk; hanya 'omzet' = Omset)
  ...
  bd AS (
    SELECT
      ...
      (SELECT COALESCE(sum(init_paid),0) FROM txall WHERE payment_method IN ('cash','hutang')) + (SELECT COALESCE(sum(round(amount)),0) FROM dpall WHERE payment_method='cash') + (SELECT COALESCE(sum(round(amount)),0) FROM ecpall WHERE payment_method='cash') + (SELECT COALESCE(sum(round(amount)),0) FROM oiall WHERE method='cash') + (SELECT COALESCE(sum(round(amount)),0) FROM cbiall WHERE payment_method='cash') AS masuk_cash,
      (SELECT COALESCE(sum(init_paid),0) FROM txall WHERE payment_method='transfer') + (SELECT COALESCE(sum(round(amount)),0) FROM dpall WHERE payment_method='transfer') + (SELECT COALESCE(sum(round(amount)),0) FROM ecpall WHERE payment_method='transfer') + (SELECT COALESCE(sum(round(amount)),0) FROM oiall WHERE method='transfer') + (SELECT COALESCE(sum(round(amount)),0) FROM cbiall WHERE payment_method='transfer') AS masuk_transfer,
      (SELECT COALESCE(sum(init_paid),0) FROM txall WHERE payment_method='qris') + (SELECT COALESCE(sum(round(amount)),0) FROM dpall WHERE payment_method='qris') + (SELECT COALESCE(sum(round(amount)),0) FROM oiall WHERE method='qris') + (SELECT COALESCE(sum(round(amount)),0) FROM cbiall WHERE payment_method='qris') AS masuk_qris,
  ...
  SELECT json_build_object(
    -- Omset = invoice kasir + migrasi omset lama + Credibook jenis 'omzet' SAJA.
