...
CREATE TABLE IF NOT EXISTS public.credibook_income (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name             text NOT NULL,
  transaction_date date NOT NULL,
  amount           numeric NOT NULL DEFAULT 0,
  payment_method   text NOT NULL DEFAULT 'transfer',  -- cash | transfer | qris
  note             text,
  book_id          uuid,
  created_by       uuid,
...
CREATE INDEX IF NOT EXISTS idx_credibook_income_deleted ON public.credibook_income (deleted_at);
CREATE INDEX IF NOT EXISTS idx_credibook_income_date    ON public.credibook_income (transaction_date);
CREATE INDEX IF NOT EXISTS idx_credibook_income_book    ON public.credibook_income (book_id);

ALTER TABLE public.credibook_income ENABLE ROW LEVEL SECURITY;
DO $$ BEGIN CREATE POLICY "anon all credibook_income" ON public.credibook_income FOR ALL USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
GRANT ALL ON public.credibook_income TO anon, authenticated;

-- Backfill book_id data lama → SKUPY (jika tabel books ada).
DO $do$
DECLARE v_skupy uuid;
BEGIN
  SELECT id INTO v_skupy FROM public.books WHERE upper(name)='SKUPY' AND deleted_at IS NULL LIMIT 1;
  IF v_skupy IS NOT NULL THEN
    UPDATE public.credibook_income SET book_id = v_skupy WHERE book_id IS NULL;
  END IF;
