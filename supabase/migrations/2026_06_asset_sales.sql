-- =====================================================================
-- Skupy POS — PENJUALAN ASET (asset_sales) + sinkron ke Kas/Saldo/Arus Kas
-- =====================================================================
-- Saat aset dijual: harga jual MASUK kas/bank (bukan Omset). Aset Tetap turun
-- otomatis karena status='sold'. Untung/Rugi = harga jual − nilai buku.
-- Additive + idempotent. Tidak mengubah rumus laba/omset. payment_method
-- DISIMPAN di asset_sales (BUKAN di tabel assets).
-- =====================================================================

-- Pastikan kolom status/sold ada di assets (tanpa payment_method wajib).
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='assets') THEN
    BEGIN ALTER TABLE public.assets ADD COLUMN IF NOT EXISTS status text DEFAULT 'active'; EXCEPTION WHEN others THEN NULL; END;
    BEGIN ALTER TABLE public.assets ADD COLUMN IF NOT EXISTS sold_date date; EXCEPTION WHEN others THEN NULL; END;
    BEGIN ALTER TABLE public.assets ADD COLUMN IF NOT EXISTS sold_price numeric; EXCEPTION WHEN others THEN NULL; END;
  END IF;
END $$;
...
DO $$ BEGIN CREATE POLICY "anon all asset_sales" ON public.asset_sales FOR ALL USING (true) WITH CHECK (true); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
GRANT ALL ON public.asset_sales TO anon, authenticated;
CREATE INDEX IF NOT EXISTS idx_asset_sales_asset ON public.asset_sales(asset_id) WHERE deleted_at IS NULL;

-- ───── acc_dashboard: tambah penjualan aset ke KAS MASUK (bukan Omset) ─────
...
RETURNS json LANGUAGE sql STABLE AS $$
  ...
  bd AS (
    SELECT
      ...
      (SELECT COALESCE(sum(init_paid),0) FROM txall WHERE payment_method='qris') + (SELECT COALESCE(sum(round(amount)),0) FROM dpall WHERE payment_method='qris') + (SELECT COALESCE(sum(round(amount)),0) FROM oiall WHERE method='qris') + (SELECT COALESCE(sum(round(amount)),0) FROM cbiall WHERE payment_method='qris') + (SELECT COALESCE(sum(round(sale_price)),0) FROM aslall WHERE payment_method='qris') AS masuk_qris,
